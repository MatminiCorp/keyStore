...
public class PasswordEditWizardDialog extends JDialog {
  ...
  private JButton btnPrevious = new JButton(
      ConstantsParameters.ACTION_BUTTON_PREVIOUS);
  private JButton btnUpdate = new JButton(
      ConstantsParameters.ACTION_BUTTON_UPDATE);
  private JButton btnCancel = new JButton(
      ConstantsParameters.ACTION_BUTTON_CANCEL);
  
  private int currentStep = 0;
  
  private final Map<String, String> fieldValues = new HashMap<>();
  private final String[] steps = {SimpleKeyStoreTableEnum.NAME.getColumn(),
      SimpleKeyStoreTableEnum.URL.getColumn(),
      SimpleKeyStoreTableEnum.USERNAME.getColumn(),
      SimpleKeyStoreTableEnum.PASSWORD.getColumn(),
      ConstantsParameters.J_LABEL_CONFIRM_PASSWORD,
      SimpleKeyStoreTableEnum.NOTE.getColumn()};
  private final Map<String, JTextField> inputs = new HashMap<>();
  private final Consumer<Map<String, String>> onConfirm;
  
  public PasswordEditWizardDialog(Frame owner,
  ...
  private JPanel buildFieldPanel(String field, String initialValue) {
    ...
    JTextField input = SimpleKeyStoreTableEnum.PASSWORD.getColumn().equals(
        field) || ConstantsParameters.J_LABEL_CONFIRM_PASSWORD.equals(field)
    ...
    btnNext.addActionListener(e -> {
      ...
      if (currentStep < steps.length - 1) {
        ...
        if (!fieldValues.get(SimpleKeyStoreTableEnum.PASSWORD.getColumn())
            .equals(fieldValues
                .get(ConstantsParameters.J_LABEL_CONFIRM_PASSWORD))) {
  ...
  private boolean captureCurrentField() {
    ...
    if (value == null || value.trim().isEmpty()) {
      JOptionPane.showMessageDialog(this, step + " is required", "Validation",
