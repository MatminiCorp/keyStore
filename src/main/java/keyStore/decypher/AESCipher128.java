...
public class AESCipher128 {
	...
	public String encrypt(String input) throws Exception {
		...
		byte[] cipherWithSaltIv = new byte[salt.length + iv.getIV().length + cipherText.length];
		System.arraycopy(salt, 0, cipherWithSaltIv, 0, salt.length);
		System.arraycopy(iv.getIV(), 0, cipherWithSaltIv, salt.length, iv.getIV().length);
		System.arraycopy(cipherText, 0, cipherWithSaltIv, salt.length + iv.getIV().length, cipherText.length);
	...
	public String decrypt(String cipherText) throws Exception {
		...
		byte[] actualCipherText = Arrays.copyOfRange(decodedCipherText, 32, decodedCipherText.length);
		SecretKey key = generateSecretKey(secret, salt);
		Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
